...
// Every OTHER game (Lorcana, One Piece, Riftbound, Digimon, …) plays on the
// universal mana system: cost scales with the card's power, summoning
// sickness applies. Power comes from real stats where the game has them,
// else from the card's rarity tier (stamped at seed/patch time as bs).
...
// mana is the universal cost system: Magic and the eleven non-trio games
// all pay it (Pokémon runs on energy, Yu-Gi-Oh on tributes)
export const usesMana = (c) => c.game !== "pokemon" && c.game !== "yugioh";
// the universal stat→mana curve every game without a printed cost pays.
// Stats are each card's PRINTED numbers (decimal-shifted only), so games
// live at different magnitudes — a Digimon is a 60/60, a DB Fusion card a
// 20/20. Each game converts stats to mana at its own exchange rate,
// calibrated so no game's bodies dominate the 5-slot board.
const MANA_RATE = {
  digimon: 19, onepiece: 19, riftbound: 18, swu: 19, weiss: 20,
  lorcana: 20, gundam: 20, fab: 20, netrunner: 19, unionarena: 20,
  dbfusion: 44, mtg: 22,
};
const curveCost = (c) => Math.max(1, Math.min(MANA_MAX, Math.ceil((c.bs[0] + c.bs[1]) / (MANA_RATE[c.game] || 20))));
// real printed CMC when the card carries it (mc) — but never priced worse
// than its body's curve cost: real cards pay for rules text we don't
// simulate, so a 2/2-for-4 plays here as a 2/2-for-3 like everyone else's
export const manaCost = (c) => Number.isFinite(c.mc) && Array.isArray(c.bs)
  ? Math.min(Math.max(1, Math.min(MANA_MAX, c.mc)), curveCost(c))
  : Number.isFinite(c.mc) ? Math.max(1, Math.min(MANA_MAX, c.mc)) : curveCost(c);
export const energyNeed = (c) => Math.max(1, Math.min(3, Math.ceil(c.bs[0] / 25)));

// Pokémon fight with their REAL printed attacks (atks: [name, dmg, cost]) —
// attached energy unlocks them tier by tier. A slot's effective ATK is the
// best attack it can currently afford; unpowered Pokémon can still block,
// they just hit back for nothing.
export const pkmAtks = (c) => (c.game === "pokemon" && Array.isArray(c.atks) && c.atks.length ? c.atks : null);
export function bestAttack(b) {
  const list = pkmAtks(b.card);
  if (!list) return null;
  let best = null;
  for (const a of list) if (a[2] <= b.energy && (!best || a[1] > best[1])) best = a;
  return best;
}
export function slotAtk(b) {
  const list = pkmAtks(b.card);
  if (!list) return b.card.bs[0];
  const a = bestAttack(b);
  return a ? a[1] : 0;
}
// Pokémon attacks exploit weakness: double damage TO CREATURES (their
// printed numbers run a decade under everyone else's) — face damage is
// unaffected
export const creatureAtk = (b) => (pkmAtks(b.card) ? slotAtk(b) * 2 : slotAtk(b));
// cheapest attack = what it takes to swing at all; dearest = full charge
export const minAtkCost = (c) => { const l = pkmAtks(c); return l ? Math.min(...l.map((a) => a[2])) : energyNeed(c); };
export const maxAtkCost = (c) => { const l = pkmAtks(c); return l ? Math.max(...l.map((a) => a[2])) : energyNeed(c); };
// what a BLOCKER hits back with: Pokémon use their charged attack, everyone
// else swings with printed ATK
export const blockAtk = (b) => (pkmAtks(b.card) ? slotAtk(b) : b.card.bs[0]);
// stats are printed ATK/DEF ÷ 100: Blue-Eyes (30+25) asks one tribute,
// only the true god cards ask two
export const tributeNeed = (c) => { const s = c.bs[0] + c.bs[1]; return s >= 85 ? 2 : s >= 50 ? 1 : 0; };
...
export function costOf(c) {
  ...
  if (c.game === "yugioh") return { kind: "tribute", n: tributeNeed(c) };
  return { kind: "mana", n: manaCost(c) };
}
...
// Pokémon arrive with 1 energy attached — cheap attacks come online right
// away, the big ones still need charging.
// Pokémon arrive with 2 energy — printed-number bodies run small, so they
// come in semi-charged and lean on tempo
const slot = (card) => ({ card, curHp: card.bs[1], energy: card.game === "pokemon" ? 3 : 0, sick: usesMana(card), attacked: false, uid: Math.random().toString(36).slice(2, 9) });
...
export function initDuel(playerDeck, aiDeck, rng = Math.random, names = null) {
  ...
  const side = (deck) => ({ hp: PLAYER_HP, deck: shuffle(deck), hand: [], board: [], turns: 0, mana: 0, maxMana: 0, energyUsed: false, summonUsed: false, specialUsed: false, trainerUsed: false, spellUsed: false });
...
export function canPlay(state, who, i) {
  ...
  if (c.game === "yugioh") {
    // one normal summon per turn, plus one SPECIAL summon of a genuinely
    // small monster (printed ATK+DEF under 3000) — the swarm identity
    if (tributeNeed(c) > s.board.length) return false;
    if (!s.summonUsed) return true;
    return !s.specialUsed && c.bs[0] + c.bs[1] < 25;
  }
  return manaCost(c) <= s.mana; // mtg + every non-trio game
}
...
function castSpell(state, who, i, spellTarget) {
  ...
  if (kind === "dmg" && t === "face") {
    ...
    const tgt = o.board[t];
    const wk = (l) => (l ? l.map(([an, ad, ac]) => [an, Math.max(1, ad - n), ac]) : undefined);
    tgt.card = { ...tgt.card, bs: [Math.max(1, tgt.card.bs[0] - n), tgt.card.bs[1]], ...(pkmAtks(tgt.card) && { atks: wk(tgt.card.atks) }) };
    say(state, `${FX_LABEL[kind]} ${c.name}: ${tgt.card.name} loses ${n}⚔`);
  } else if (kind === "buff") {
    const tgt = s.board[t];
    const bf = (l) => (l ? l.map(([an, ad, ac]) => [an, Math.min(120, ad + n), ac]) : undefined);
    tgt.card = { ...tgt.card, bs: [Math.min(120, tgt.card.bs[0] + n), Math.min(120, tgt.card.bs[1] + n)], ...(pkmAtks(tgt.card) && { atks: bf(tgt.card.atks) }) };
    tgt.curHp += n;
...
export function playCard(state, who, i, tributes = [], evoTarget = null, spellTarget = null) {
  ...
  if (usesMana(c)) s.mana -= manaCost(c);
  if (c.game === "yugioh") {
    ...
    if (!s.summonUsed) s.summonUsed = true;
    else s.specialUsed = true;
  }
  s.hand.splice(i, 1);
  s.board.push(slot(c));
...
export function canAttach(state, who, bi) {
  ...
  return !!b && !state.over && state.active === who && state.phase !== "block" && !s.energyUsed &&
    b.card.game === "pokemon" && b.energy < maxAtkCost(b.card);
}
...
export function attachEnergy(state, who, bi) {
  ...
  b.energy++;
  s.energyUsed = true;
  const mv = bestAttack(b);
  say(state, `⚡ energy on ${b.card.name} (${b.energy}/${maxAtkCost(b.card)})${mv ? ` — ${mv[0]} ready` : ""}`);
  return true;
}
...
export function canAttackWith(state, who, bi) {
  ...
  if (b.sick) return false;
  if (b.card.game === "pokemon") {
    if (pkmAtks(b.card)) return slotAtk(b) > 0; // can afford SOME attack
    if (b.energy < energyNeed(b.card)) return false;
  }
  return true;
}
...
export function resolveCombat(state, blocks = {}) {
  ...
  for (const a of state.combat.attackers) {
    ...
    const move = bestAttack(atk); // Pokémon announce their real attack
    const atkDmg = slotAtk(atk);
    const atkName = move ? `${atk.card.name} uses ${move[0]}` : atk.card.name;
    const blk = clean[a] !== undefined ? o.board[clean[a]] : null;
    if (blk && blk.curHp > 0) {
      say(state, `🛡 ${blk.card.name} blocks ${move ? `${atk.card.name} (${move[0]})` : atk.card.name}`);
      blk.curHp -= creatureAtk(atk);
      atk.curHp -= pkmAtks(blk.card) ? blockAtk(blk) * 2 : blockAtk(blk);
    } else {
      const d = Math.ceil(atkDmg / 10);
      o.hp -= d;
      say(state, `${atkName} — hits ${hitName(state, foe)} for ${d}`);
    }
  }
  cull(state, who);
  cull(state, foe);
...
export function aiStep(state, who) {
  ...
  // 3. Yu-Gi-Oh: best summon (normal, then a small special summon),
  // tributing only when it upgrades the board
  {
    const picks = s.hand
      .map((c, i) => ({ c, i }))
    ...
    for (const { c, i } of picks) {
      ...
      if ((need === 0 || statSum(c) > cost + 10) && playCard(state, who, i, fodder.map((f) => f.x))) return true;
    }
  }
  // 4. mana games (Magic + the other eleven): biggest affordable creature
  const mg = s.hand.map((c, i) => ({ c, i }))
    .filter(({ c, i }) => usesMana(c) && !isSpell(c) && canPlay(state, who, i))
  ...
  // 6. energy: charge the Pokémon with the biggest attack still locked,
  // preferring whoever is closest to its next power spike
  const en = s.board.map((b, x) => ({ b, x }))
    .filter(({ x }) => canAttach(state, who, x))
    .sort((a, b) => (maxAtkCost(a.b.card) - a.b.energy) - (maxAtkCost(b.b.card) - b.b.energy) || b.b.card.bs[0] - a.b.card.bs[0]);
...
export function aiBlocks(state) {
  ...
  const attackers = state.combat.attackers
    .filter((a) => s.board[a])
    .sort((x, y) => slotAtk(s.board[y]) - slotAtk(s.board[x]));
  let incoming = attackers.reduce((n, a) => n + Math.ceil(slotAtk(s.board[a]) / 10), 0);
  const free = () => o.board.map((b, x) => ({ b, x })).filter(({ x }) => !used.has(x));
  const retaliation = (b) => (pkmAtks(b.card) ? blockAtk(b) * 2 : blockAtk(b));
  for (const a of attackers) {
    const atk = s.board[a];
    const atkDmg = creatureAtk(atk);
    // kills the attacker and survives
    let pick = free()
      .filter(({ b }) => retaliation(b) >= atk.curHp && b.curHp > atkDmg)
      .sort((p, q) => statSum(p.b.card) - statSum(q.b.card))[0];
    // trades: kills the attacker, dies, but the attacker was worth it
    if (!pick) pick = free()
      .filter(({ b }) => retaliation(b) >= atk.curHp && statSum(atk.card) >= statSum(b.card))
      .sort((p, q) => statSum(p.b.card) - statSum(q.b.card))[0];
    ...
    if (pick) { blocks[a] = pick.x; used.add(pick.x); incoming -= Math.ceil(atkDmg / 10); }
  }
  return blocks;
}
