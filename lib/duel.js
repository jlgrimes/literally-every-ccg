...
const MANA_RATE = {
  digimon: 18, onepiece: 19, riftbound: 18, swu: 18, weiss: 19,
  lorcana: 19, gundam: 19, fab: 19, netrunner: 19, unionarena: 20,
  dbfusion: 40, mtg: 26,
};
...
export function canPlay(state, who, i) {
  ...
  if (c.game === "yugioh") {
    // one normal summon per turn, plus one SPECIAL summon of a small
    // (tribute-free) monster — Yu-Gi-Oh's swarm-the-board identity
    if (tributeNeed(c) > s.board.length) return false;
    if (!s.summonUsed) return true;
    return !s.specialUsed && tributeNeed(c) === 0;
  }
