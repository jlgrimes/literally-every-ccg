...
const curveCost = (c) => Math.max(1, Math.min(MANA_MAX, Math.ceil((c.bs[0] + c.bs[1]) / 20)));
...
export const tributeNeed = (c) => { const s = c.bs[0] + c.bs[1]; return s >= 110 ? 2 : s >= 70 ? 1 : 0; };
...
const slot = (card) => ({ card, curHp: card.bs[1], energy: card.game === "pokemon" ? 1 : 0, sick: usesMana(card), attacked: false, uid: Math.random().toString(36).slice(2, 9) });
...
export function canPlay(state, who, i) {
  ...
  if (c.game === "yugioh") return !s.summonUsed && tributeNeed(c) <= s.board.length;
...
export function aiBlocks(state) {
  ...
  for (const a of attackers) {
    ...
    let pick = free()
      .filter(({ b }) => blockAtk(b) >= atk.curHp && b.curHp > atkDmg)
      .sort((p, q) => statSum(p.b.card) - statSum(q.b.card))[0];
    // trades: kills the attacker, dies, but the attacker was worth it
    if (!pick) pick = free()
      .filter(({ b }) => blockAtk(b) >= atk.curHp && statSum(atk.card) >= statSum(b.card))
      .sort((p, q) => statSum(p.b.card) - statSum(q.b.card))[0];
